...
export default function SidePanel(props: { lat: number; lon: number }) {
  return (
    <div className="fixed z-20 top-0 right-0 h-screen w-[var(--sidebar-width)] bg-light-background dark:bg-background flex flex-col gap-6 p-4 py-8 translate-x-full sm:translate-x-0 transition-transform duration-300 overflow-y-scroll">
        ...
        {Object.entries(airPollutionData.list[0].components).map(
          ([key, value]) => {
            ...
            return (
              <div
                key={key}
                className="space-y-3 dark:bg-background-secondary/50 bg-light-background-secondary/50 shadow-md rounded-lg p-4 hover:scale-105 transition-all duration-300"
              >
